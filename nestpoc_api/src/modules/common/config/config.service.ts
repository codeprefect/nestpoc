...
export class ConfigService
  implements JwtOptionsFactory, GqlOptionsFactory, TypeOrmOptionsFactory
{
  private envConfig: IEnvConfig = {};

  constructor(@Inject(envProviderKey) filePath: string) {
  ...
  private validateInput(envConfig: IEnvConfig): IEnvConfig {
    ...
    const { error, value: validatedEnvConfig } =
      defaultSchema.validate(envConfig);
  ...
  private get isDevelopment(): boolean {
    ...
    return env != null
      ? ['DEVELOPMENT', 'DEV', 'TEST'].includes(env.toLocaleUpperCase())
      : false;
  }

  createTypeOrmOptions(): TypeOrmModuleOptions | Promise<TypeOrmModuleOptions> {
    const fileExt = '{ts,js}';
    const entitiesPath = `${__dirname}/**/modules/**/models/*.model.${fileExt}`;
    const migrationsPath = `${__dirname}/**/migrations/*.${fileExt}`;
    return {
      type: this.envConfig.NESTPOC_DB_TYPE == 'mssql' ? 'mssql' : 'postgres',
      ...
      secretOrKeyProvider: (requestType: JwtSecretRequestType) => {
        switch (requestType) {
          ...
          case JwtSecretRequestType.VERIFY:
            return (
              this.envConfig.NESTPOC_JWT_PUBLIC_KEY ||
              this.envConfig.NESTPOC_JWT_PRIVATE_KEY
            );
          default:
            return this.envConfig.NESTPOC_JWT_PRIVATE_KEY;
        }
  ...
  public createAfricasTalkingOptions(): IAfricasTalkingOptions {
    ...
    if (options.options.username == null || options.options.apiKey == null) {
      console.log(
        'AfricasTalking configuration is not complete, you may not be able to send SMS',
      );
      return null;
    }
    return options;
